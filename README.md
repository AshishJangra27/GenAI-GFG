...
| **4** | Deep Learning → Generative AI & LLMs | Roadmap from Deep Learning to Generative AI, how LLMs are trained, decoding GPT models, pretraining, fine-tuning, and the complete LLM pipeline explained visually. | 🔗 [🧠 Open Excalidraw](https://excalidraw.com/#json=CitXqx1uWGzfLPvnTiBnv,A43TWw8QyBK0DLNG7ipWCQ) |
...
| **8** | DIY RAG (From Scratch) | Built Retrieval-Augmented Generation without using LangChain or fancy libraries. Implemented embeddings, similarity search using NumPy, and the LLM response pipeline manually. | 🔗 [📓 Open in Colab](https://colab.research.google.com/drive/1NJ67hZiQ5NLCFj78CrAiKNbCNvdRlvaf?usp=sharing) |
| **9** | LLM-Based Web Search Agent | Built a web search system using Serper API where multiple webpages are fetched, summarized, reasoned upon, and a final structured answer is generated by the LLM. | 🔗 [📓 Open in Colab](https://colab.research.google.com/drive/1F1_TP5RTSfsYaMD_Qpg0K4LX4tIi-y0R?usp=sharing) |
